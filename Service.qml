...
Item {
  ...
  // Injected by the shell when the plugin declares them. The registry owns
  // the parsed shell.json; this plugin only reads its own entry out of it.
  property var manifest: null
  property var pluginRegistry: null

  // This plugin's entry in shell.json's `plugins` array, which doubles as its
  // settings object the way a bar widget's entry does. Missing or unreadable
  // leaves it empty, and LockView falls back to its built-in defaults.
  property var pluginSettings: ({})

  function refreshSettings() {
    var id = manifest && manifest.id ? String(manifest.id) : ""
    var provider = pluginRegistry ? pluginRegistry.shellConfigProvider : null
  ...
  // pluginRegistry is injected last, so manifest is already in place by the
  // time this fires. A rescan is the shell re-reading shell.json, which is
  // the moment new settings can have appeared.
  onPluginRegistryChanged: refreshSettings()

  Connections {
    target: root.pluginRegistry
    ignoreUnknownSignals: true
    function onRegistryRevisionChanged() { root.refreshSettings() }
  }
